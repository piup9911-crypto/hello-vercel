...
const CODEX_STATUS_KEY = "codex_status";
...
async function readCodexStatus(config) {
...
async function writeCodexStatus(config, status, userId) {
  const rows = await callSupabase(
    config,
...
export async function GET(request) {
  ...
  try {
    return json(200, await readCodexStatus(config));
  } catch (error) {
    return json(500, {
...
export async function PUT(request) {
  ...
  try {
    ...
    return json(200, await writeCodexStatus(config, status, auth.userId));
  } catch (error) {
    return json(500, {
      error: error && error.message ? error.message : String(error)
    });
